...
all_ret = merge.xts(tmp3,ri_nflx,join="inner")
...
beta_goog = round(fit_goog$coefficients[2],2)
beta_meta = round(fit_meta$coefficients[2],2)
beta_nflx = round(fit_nflx$coefficients[2],2)
alpha_goog = round(fit_goog$coefficients[1],2)
alpha_meta = round(fit_meta$coefficients[1],2)
alpha_nflx = round(fit_nflx$coefficients[1],2)

alpha_beta = matrix(c(alpha_goog, alpha_meta, alpha_nflx, beta_goog, beta_meta, beta_nflx),2,3,byrow = TRUE)
colnames(alpha_beta) = c("GOOG", "META", "NFLX")
rownames(alpha_beta) = c("Alpha", "Beta")
...
rm = monthlyReturn(sp500_new$gspc.adjusted, type = "arithmetic")
names(rm) = c("mkt")
...
tmp1 = merge.xts(rm,rf_new,join="inner")
tmp2 = merge.xts(tmp1,ri_goog_new,join="inner")
tmp3 = merge.xts(tmp2,ri_meta_new,join="inner")
all_ret_new = merge.xts(tmp3,ri_nflx_new,join="inner")
...
fit_goog_new = lm(ex_goog_new~ex_mkt)
fit_meta_new = lm(ex_meta_new~ex_mkt)
fit_nflx_new = lm(ex_nflx_new~ex_mkt)

beta_goog_new = round(fit_goog_new$coefficients[2],2)
beta_meta_new = round(fit_meta_new$coefficients[2],2)
beta_nflx_new = round(fit_nflx_new$coefficients[2],2)
alpha_goog_new = round(fit_goog_new$coefficients[1],6)
alpha_meta_new = round(fit_meta_new$coefficients[1],6)
alpha_nflx_new = round(fit_nflx_new$coefficients[1],6)

alpha_beta_new = matrix(c(alpha_goog_new, alpha_meta_new, alpha_nflx_new, beta_goog_new, beta_meta_new, beta_nflx_new),2,3,byrow = TRUE)
colnames(alpha_beta_new) = c("GOOG", "META", "NFLX")
rownames(alpha_beta_new) = c("Alpha", "Beta")
